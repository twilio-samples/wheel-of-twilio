...
test.describe("Prize Inventory System", () => {
  test("Prize inventory disabled when NEXT_PUBLIC_PRIZES_PER_FIELD is not set", async ({
    page,
  }) => {
  ...
  test("Visual indicators for fields with different prize states", async ({
    page,
  }) => {
    const prizesPerField = process.env.NEXT_PUBLIC_PRIZES_PER_FIELD;
  ...
  test("Spin button and core functionality remain unaffected", async ({
    page,
  }) => {
    await page.goto(
      `http://${BASIC_AUTH_USERNAME}:${BASIC_AUTH_PASSWORD}@localhost:3000/`,
